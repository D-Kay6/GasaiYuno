...
namespace GasaiYuno.Discord.Commands.Modules.Entertainment
{
    ...
    {
        private readonly IUnitOfWork<ICommandRepository> _repository;

        public CustomCommandModule(IUnitOfWork<ICommandRepository> repository)
        {
        ...
        {
            var commands = await _repository.DataSet.CountAsync(x => x.Server.Id == Context.Guild.Id);
        ...
        {
            var commands = await _repository.DataSet.ListAsync(Context.Guild.Id).ConfigureAwait(false);
        ...
        {
            var conflictingCommand = await _repository.DataSet.GetAsync(Context.Guild.Id, command).ConfigureAwait(false);
            ...
            _repository.DataSet.Add(customCommand);
        ...
        {
            var customCommand = await _repository.DataSet.GetAsync(Context.Guild.Id, command.Trim()).ConfigureAwait(false);
            if (customCommand == null)
            {
                await ReplyAsync(Translation.Message("Entertainment.CustomCommand.Invalid.None", command)).ConfigureAwait(false);
                return;
            }

            await _repository.BeginAsync().ConfigureAwait(false);
            _repository.DataSet.Remove(customCommand);
            await _repository.SaveAsync().ConfigureAwait(false);
