...
namespace GasaiYuno.Discord.Listeners
{
    public class SlashCommandListener
    {
...
#if DEBUG
                await _interactionService.RegisterCommandsToGuildAsync(571289745049780224).ConfigureAwait(false);
                await _interactionService.RegisterCommandsToGuildAsync(274638156966526977).ConfigureAwait(false);
#else
...
#if DEBUG
                if (!interaction.User.Id.Equals(255453041531158538))
                {
        ...
        private async Task OnSlashCommandExecuted(SlashCommandInfo commandInfo, IInteractionContext context, IResult result)
        {
            ...
            _logger.LogInformation("Execution of {Command} failed with {Error} {Reason}.", commandInfo, result.Error, result.ErrorReason);
